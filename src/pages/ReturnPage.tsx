...
const ReturnPage: React.FC = () => {
  ...
  useEffect(() => {
    const entriesRef = firestore
      .collection("users")
      .doc(userId)
      .collection("Orders").where("Status", "==", "Return");
    ...
    <IonPage className="dashboard-page">
      <IonHeader>
        <IonToolbar color="deliverboy">
          ...
          <IonTitle>Deliveries Returns</IonTitle>
        </IonToolbar>
      </IonHeader>

      <IonContent>
        <IonList color="dark" mode="ios">
          <IonItem color="danger" mode="ios">
            <strong>Returns</strong>
          </IonItem>
        </IonList>

        {entries.map((entry) => (<IonCard color="dark" button key={entry.id} routerLink={`./returnpageentry/${entry.id}`}>
          <IonCardHeader>
            <IonGrid>
              <IonRow>
                <IonCol size="10">
